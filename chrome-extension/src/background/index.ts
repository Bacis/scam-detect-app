...
async function promptLanguageModel(promptText: string): Promise<string> {
  ...
  try {
    const instruction = `Analyze the following YouTube short transcript: ${promptText}. Provide the response in JSON format with a scam rating in a decimal value, where 1 is an absolute scam and closer to 0 is legitimate. Also, I want you to structure the response in a way that is easy to parse and extract the keys: scam_rating, phrases_intents, and explanation.`;
